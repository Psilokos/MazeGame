...
// Last update Thu Jan  5 19:24:02 2017 Victorien LE COUVIOUR--TUFFET
//
...
namespace	compile_time
{
  ...
  // Some kind of tuple (check why I did that)
  ...
  // Gets the index of a char const * in a char const * parameter pack
  ...
  // Checks if all types in a template parameter pack are the same

  template<bool...> struct	BoolPack;

  template<bool... boolPack>
  using AllTrue = std::is_same<BoolPack<true, boolPack...>, BoolPack<boolPack..., true>>;

  template<typename T, typename... TPack>
  using AreAllConvertible = AllTrue<std::is_convertible<TPack, T>::value...>;
}
